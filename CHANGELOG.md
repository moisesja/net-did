...
- **Verification-relationship authorization primitive** (#71): New `IVerificationRelationshipResolver` (in `NetDid.Core.Resolution`) answers "does the _controller's_ DID document authorize this verification method for this verification relationship?" — the W3C DID Core §5.3 check that a correct ZCAP-LD invocation/delegation verifier needs but cannot derive from a `bareDid(verificationMethod) == controller` string heuristic. The heuristic is sound only for `did:key`; it admits two real break cases on richer methods: (A) cross-DID references, where the relationship list authorizes a key whose base DID differs from the controller (e.g. `did:web:example.com`'s `capabilityInvocation` references `did:web:alice.example#key-1`), and (B) per-purpose key separation, where one controller exposes a hot key under `capabilityInvocation` and a cold key under `capabilityDelegation` — a string heuristic authorizes both keys for both purposes, defeating the operator's deliberate split. The new primitive resolves the controller's document via `IDidResolver`, looks up the relationship list, and normalizes both sides (fragment-only `"#k1"` and bare `"k1"` against the controller's id) before an ordinal compare. Returns a tri-state `VerificationRelationshipAuthorizationResult { Decision, ResolutionError, Message }` with decisions `Authorized` / `NotAuthorized` / `ControllerNotResolvable` — the third state surfaces the underlying resolution error code (`notFound`, `invalidDid`, `methodNotSupported`) so verifiers can fail closed _and_ log the infrastructure cause rather than silently collapsing both into denial. Registered as a singleton by `services.AddNetDid(...)`. The relationship-list selector formerly held privately inside `DefaultDidUrlDereferencer` is promoted to a public extension method `DidDocument.GetRelationshipEntries(this DidDocument, VerificationRelationship)` (plus a wire-name string overload) sharing the new `VerificationRelationship` enum and `VerificationRelationshipNames` constants; the dereferencer now consumes the extension so there is a single source of truth. Behavior-identical refactor — all 807 existing tests still pass. Unblocks `zcap-dotnet#65` controller-authorization soundness for non-`did:key` methods. Test coverage: 22 unit tests (cross-DID reference, relationship discrimination, embedded entry match, both normalization forms, missing/empty relationship list, all three resolution-error variants plus an unset-error fallback, normalized foreign-VM mismatch, query-param non-stripping, null/whitespace argument guards) plus 10 `did:key` regression tests asserting Ed25519 primary VM authorizes for the four signing relationships and X25519 authorizes only for `KeyAgreement`.
