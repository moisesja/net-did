...
- **Time**: `Timeout` (default 30 seconds) bounds the total wall-clock time of each fetch — response
  ...
  `HttpClient.Timeout` does not cover under `ResponseHeadersRead` (issue #80). A timed-out fetch is
  a failed fetch (resolution reports `notFound`); cancellation via the caller's own token still
  propagates as `OperationCanceledException` (issue #81 contract).
