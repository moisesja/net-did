...
Complete proofs produced outside NetDid MUST enter the public authoring surface through
`DataIntegrityProofValue.Parse(string)` or `DataIntegrityProofValue.FromJson(JsonElement)`. Both
factories reject duplicate JSON members recursively, retain the complete proof object for signing
and publication fidelity, and derive the modeled type, cryptosuite, verification method, created
time, purpose, and proof value from that same object. `RawJson` remains internally settable only, so
a caller cannot construct a raw/modeled dual state with different authorization fields.
...
8. Determine the witness authority for every entry. Genesis is governed by the witness policy it declares. The first later entry that changes from no active witnesses to a configured policy is immediately governed by that new policy and MUST itself be witnessed. Once a policy is active, it governs the entry that lowers, removes, or replaces it, and the replacement takes effect only after that entry is published. If any entry through the requested version requires witnessing, fetch `did-witness.json` and validate cumulative witness coverage against those authorizing policies. `did-witness.json` is a JSON array of `{ "versionId": ..., "proof": [...] }` entries (the member is `proof`, singular, per did:webvh v1.0 — issue #135), parsed strictly: UTF-8, duplicate JSON members rejected, and a parse failure is logged with its reason before resolution reports `witnessValidationFailed`.

   Each witness proof is an `eddsa-jcs-2022` Data Integrity proof with `proofPurpose: assertionMethod` and a verificationMethod in the strict `did:key:<multibase>#<multibase>` form (the bare-DID form is not counted — conforming resolvers discard it), whose input document is `{"versionId": "<versionId>"}` — not the log entry itself. The `versionId` embeds the entry hash that chain validation independently recomputes before witness validation runs, so an approval still binds the full entry content, and the input document is always built from the chain-validated entry's `versionId`, never from the witness file's claimed key. Proofs are verified with their complete wire configuration through the Data Integrity pipeline: signature-bound members such as `expires` and extensions are honored, and an unsigned injected member fails verification. A candidate with `previousProof` is verified with the complete dependency closure found in the same version bucket. Missing, duplicate/ambiguous, repeated, or cyclic references fail closed. A failed aggregate MUST NOT memoize an individually successful dependency as valid; positive dependency verdicts are reusable only after the entire supplied closure verifies. Dependency proofs provide Data Integrity context only: the configured candidate's signer is the sole possible approval, while an unconfigured dependency may be cryptographically verified but is never counted toward the threshold. A syntactically valid proof that cannot be represented by JCS is an invalid proof, not a resolver exception.

   A version's proofs may be split across duplicate-`versionId` entries (the ts reference implementation authors one entry per proof) and are aggregated. Semantically identical proofs are deduplicated by the JCS-canonical complete proof object, so whitespace, property order, or parsed-versus-programmatic provenance cannot multiply an approval or grow a republished artifact; signature-bound extensions remain part of identity. Proof-less entries and proofs without the optional `created` member are tolerated and simply contribute no approvals or verify without `created`, respectively. Count a configured witness only after its proof verifies and binds to the exact configured `did:key` signer (never a `verificationMethod` prefix), and count each distinct signer at most once per required entry.

   Witness validation is indexed and near-linear in the chain plus relevant proof candidates: it validates each distinct inherited policy object once, indexes configured signer membership once per validation pass, advances each signer's descending-version coverage cursor monotonically instead of rescanning every suffix, and orders only the selected dependency closure rather than rescanning the full same-version bucket. Cancellation is observed before and after bounded JSON/JCS operations and throughout library-owned indexing, policy, proof, and dependency-array loops. One resolution-local verification ledger — not a process singleton or a fresh budget per validation pass — is reused for both the requested historical prefix and the optional deactivation-tail check. Its configurable cryptographic-work budget (default 1024) charges the actual proof-closure size sent through the pipeline and fails closed when exhausted. Direct callers set both budgets with `new DidWebVhMethod(client, logger: null, maxControllerProofsPerEntry: controllerValue, maxWitnessProofVerifications: witnessValue)`; DI callers use the additive, binary-compatible `builder.AddDidWebVh(httpClientOptions: null, maxControllerProofsPerEntry: controllerValue, maxWitnessProofVerifications: witnessValue)` overload. Existing constructor and registration signatures retain the default witness budget.
...
8. If witnesses are configured, collect witness signatures into `did-witness.json` (a JSON array of `{ "versionId": ..., "proof": [...] }` entries; each proof signs the `{"versionId": "..."}` input document — issue #135).
...
3. One overall write deadline bounds the whole batch and post-transaction readback, including
   ...
   cancels a token-ignoring dependency task, every pending cancelable wait carries a
   short-lived, flow-suppressed cancellation monitor while returning bare `WaitAsync`'s task
   unchanged. If cancellation leaves the source still pending, the monitor installs one
   deduplicated source-fault observer with `ExecutionContext` flow suppressed, so a hung task
   cannot pin the abandoning request's ambient state and an orphan that faults later can never
   surface as a `TaskScheduler.UnobservedTaskException` escalation in the host. Completed tasks
   and non-cancelable waits take the bare fast path; normally completing pending waits create
   no persistent source-observer state and never touch its table or lock. Once transaction
   submission begins, every
...
- Hostile-RPC hardening: incomplete, foreign-registry, foreign-identity, duplicate-`logIndex`, `removed: true`, forward-pointing, and decreasing-timestamp histories all fail closed to `internalError` (issue #116: infrastructure/consistency failures are never `notFound`; a fixed, library-owned `message` in the resolution metadata distinguishes a pruned/incomplete history from a generic failure — exception text from injectable seams is never exposed. Legacy string error vocabulary; RFC 9457 error-object migration tracked in #123)
