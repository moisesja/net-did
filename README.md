...
NetDid authors whole-second `versionTime`s: Create truncates the current UTC instant, and a same-second update advances one whole second past the previous entry. Update fails closed if the next `versionTime` would run more than 1 minute ahead of the clock (bounding a burst at ~60 same-second writes, then ~1 write/second, and keeping a ≥4-minute margin under the at-most-5-minute future tolerance conforming resolvers enforce); Deactivate stays monotonic but is exempt from that budget so a maliciously future-dated entry can never block emergency revocation. For NetDid-authored logs the DID Core-mandated whole-second `created`/`updated` metadata therefore coincides exactly with `versionTime`. Logs imported from implementations that author fractional `versionTime`s keep full-precision reading and selection; for those logs `created`/`updated` are informational only — use `versionTime`/`versionId` as version selectors, and never feed a serialized `updated` back as a `?versionTime=` query.
