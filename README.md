...
Every supplied controller proof on an entry is verified under the full W3C Data Integrity algorithm (via DataProofsDotnet) and authorized against the active `updateKeys` (a `did:key` verificationMethod — DID==fragment anti-spoof — used for `assertionMethod`); one authorized signer authorizes the entry, and any invalid or unauthorized extra proof rejects the log as `invalidDidLog` — controller proofs have no threshold semantics. The wire `proof` may be a single object or an array, `created` is optional, and schema-defined extras (`id`, `expires`) and other Data Integrity members are preserved and validated — a `previousProof` chain is resolved (dangling references rejected) and an `expires` at or before the entry's `versionTime` is treated as expired. Duplicate JSON members and malformed content are rejected as `invalidDidLog`. Verification work per entry is bounded by a configurable controller-proof limit (default 8). Entries exposed via `DidResolutionOptions.IncludeLog` (`Artifacts["log.entries"]`) therefore carry only fully validated proofs.
