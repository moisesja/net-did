...
The result includes `Artifacts["did.jsonl"]` (the verifiable log) and `Artifacts["did.json"]` (did:web backwards-compatible document). Host these at `https://example.com/.well-known/did.jsonl` and `did.json`. The parallel `did.json` includes the derived `#files` and Linked-VP `#whois` services when the controller has not explicitly overridden them; those services are not written into the signed log state. When `WitnessProofs` are provided, a `did-witness.json` artifact is also produced. For a complete proof produced by an external witness toolchain, use `DataIntegrityProofValue.Parse(proofJson)` or `DataIntegrityProofValue.FromJson(proofElement)` before adding it: these factories preserve signature-bound members outside the baseline model, reject duplicate JSON members recursively, and derive the modeled signer/purpose fields from those same raw bytes. The raw JSON setter remains internal, preventing callers from supplying one signer in the modeled fields and different signed content for verification or publication.
...
Fetched entry hashing and controller verification retain every JSON member under `parameters` and `state`, including nested extensions that the typed DID model does not surface. This prevents a post-sign extension injection from disappearing during model reconstruction. Witness proofs sign the `{"versionId": "..."}` input document per did:webvh v1.0 (issue #135); the `versionId` embeds the entry hash recomputed by chain validation, so a witness approval binds those same members transitively. A candidate carrying `previousProof` is verified together with its complete, same-version dependency closure; dangling, ambiguous, duplicate, or cyclic references fail closed. A failed closure cannot seed a positive memo entry for an individually valid child whose ancestor failed. Only a configured witness candidate contributes an approval, although an unconfigured proof may be cryptographically verified when that configured candidate explicitly depends on it. Semantically identical proof objects are deduplicated by their JCS-canonical JSON, so raw formatting/property order and parser provenance do not create duplicate approvals or artifacts, while a signature-bound extension remains part of proof identity. A syntactically valid proof outside the JCS data model is treated as an invalid proof rather than escaping resolution as a generic error. Update and Deactivate also preserve those members in prior fetched entries; deliberate public-model mutations fall back to modeled serialization instead of stale wire data. An update that preserves the document (`NewDocument == null`) likewise carries the previous state's signed nested members into the new signed entry rather than dropping them in a modeled rewrite. A supplied `NewDocument` is deep-copied once at the start of the update, so hashing, signing, the published log, and the returned document all reflect a single snapshot — a caller collection that changes contents between reads cannot publish bytes that differ from what was signed.
...
Verification work per entry is bounded by a controller-proof limit (default 8). Witness validation builds one resolution-local index and crypto ledger, validates each repeated effective policy object once, uses indexed signer membership and monotonic coverage cursors, and materializes only the selected `previousProof` closure rather than rescanning its entire version bucket. Cancellation is observed between the bounded JSON/JCS operations and throughout library-owned collection loops. The witness-verification limit (default 1024) is shared across the requested-prefix and deactivation-tail passes of one resolution; `previousProof` dependencies are charged for every actual pipeline verification. Direct consumers configure both limits with `new DidWebVhMethod(client, logger: null, maxControllerProofsPerEntry: 16, maxWitnessProofVerifications: 2048)`; DI consumers use `builder.AddDidWebVh(httpClientOptions: null, maxControllerProofsPerEntry: 16, maxWitnessProofVerifications: 2048)`. Existing constructor and registration signatures remain source- and binary-compatible and retain the default witness budget. Raising either limit increases attacker-controlled canonicalization or signature work. With `DidResolutionOptions.IncludeLog`, latest resolution exposes the fully validated log, while historical resolution exposes only the validated prefix through the selected version.
